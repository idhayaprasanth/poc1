...
def assign_asset_sections(df):
    ...
    # Normalize common variations like "Critical Risk" or "Priority Critical".
    ...
    # Map several synonyms to the canonical section names
    risk_map = {
        "critical": "critical",
        "high": "high",
        "medium": "medium",
    ...
    # Prefer explicit bucket mapping; fallback to risk-level mapping. Ensure unmapped
    # values become proper missing (pd.NA) so downstream filters work reliably.
    ...
    # Log counts for debugging: how many completed rows were mapped to each section
    try:
        ...
        # Log a small sample of completed rows that remain unmapped to help debug
...
def _analysis_worker_thread(request_id: str, df: pd.DataFrame) -> None:
    client = DGXSparkServerClient()
    total_completed = 0
    total_failed = 0
    batch_number = 0
    ...
    with analysis_background_state["lock"]:
        analysis_background_state["df_json"] = df.to_json(date_format="iso", orient="split")
        analysis_background_state["status"] = status
        analysis_background_state["running"] = False
    # Log a final summary and sample of completed rows for debugging visibility in the UI.
    try:
...
def run_dashboard_analysis(analysis_request, json_data):
    ...
    thread = threading.Thread(target=_analysis_worker_thread, args=(request_id, df), daemon=True)
...
def show_detail(selected_asset, backdrop_click, close_click, json_data):
    ...
    risk_styles = {
        "Critical": {"bg": COLORS["high_bg"], "border": COLORS["high"], "text": COLORS["high"], "badgeBg": COLORS["high"], "badgeText": "white"},
    ...
    if False and pd.notna(row.get("threat_status")):
        children.append(section("AI Analysis", {
            "Source": row.get("ai_analysis_source", "—"),
            "Overall Risk Score": f"{float(row.get('risk_score')):.1f}/10" if pd.notna(row.get("risk_score")) else "—",
            "Overall Priority": row.get("overall_priority_level", row.get("threat_status", "—")),
            "AI Summary": row.get("ai_reason", "—"),
            "Overall Remediation": row.get("remediation", "—"),
            "Tenable Score": f"{float(row.get('tenable_risk_score')):.1f}/10" if pd.notna(row.get("tenable_risk_score")) else "—",
            "Tenable Priority": row.get("tenable_priority_level", "—"),
            "Tenable Remediation": row.get("tenable_remediation", "—"),
            "BigFix Score": f"{float(row.get('bigfix_risk_score')):.1f}/10" if pd.notna(row.get("bigfix_risk_score")) else "—",
            "BigFix Priority": row.get("bigfix_priority_level", "—"),
            "BigFix Remediation": row.get("bigfix_remediation", "—"),
            "Splunk Score": f"{float(row.get('splunk_risk_score')):.1f}/10" if pd.notna(row.get("splunk_risk_score")) else "—",
            "Splunk Priority": row.get("splunk_priority_level", "—"),
            "Splunk Remediation": row.get("splunk_remediation", "—"),
            "Defender Score": f"{float(row.get('defender_risk_score')):.1f}/10" if pd.notna(row.get("defender_risk_score")) else "—",
            "Defender Priority": row.get("defender_priority_level", "—"),
            "Defender Remediation": row.get("defender_remediation", "—"),
        }))

    def score_text(value):
        return f"{float(value):.1f}/10" if pd.notna(value) else "â€”"

    children = [
        children[0],
        section("Vulnerability (Tenable.io)", {
            "Name": row.get("vuln_name", "â€”"),
            "Severity": row.get("vuln_severity", "â€”"),
            "Description": row.get("vuln_description", "â€”"),
            "Fix": row.get("vuln_fix", "â€”"),
            "Model Risk Score": score_text(row.get("tenable_risk_score")),
            "Model Priority": row.get("tenable_priority_level", "â€”"),
            "Model Remediation": row.get("tenable_remediation", "â€”"),
        }),
        section("Threat (Microsoft Defender)", {
            "Alert": row.get("threat_alert", "â€”"),
            "File Path": row.get("threat_file_path", "â€”"),
            "Process": row.get("threat_process", "â€”"),
            "Impact": row.get("threat_impact", "â€”"),
            "Fix": row.get("threat_fix", "â€”"),
            "Model Risk Score": score_text(row.get("defender_risk_score")),
            "Model Priority": row.get("defender_priority_level", "â€”"),
            "Model Remediation": row.get("defender_remediation", "â€”"),
        }),
        section("Logs & Anomaly (Splunk)", {
            "Event": row.get("anomaly_event", "â€”"),
            "Score": row.get("source_anomaly_score", "â€”"),
            "Details": row.get("anomaly_explanation", "â€”"),
            "Model Risk Score": score_text(row.get("splunk_risk_score")),
            "Model Priority": row.get("splunk_priority_level", "â€”"),
            "Model Remediation": row.get("splunk_remediation", "â€”"),
        }),
        section("Patch Status (IBM BigFix)", {
            "Status": row.get("patch_status", "â€”"),
            "Severity": row.get("patch_severity", "â€”"),
            "Action": row.get("patch_recommendation", "â€”"),
            "Issue Status": row.get("issue_status", "â€”"),
            "Model Risk Score": score_text(row.get("bigfix_risk_score")),
            "Model Priority": row.get("bigfix_priority_level", "â€”"),
            "Model Remediation": row.get("bigfix_remediation", "â€”"),
        }),
    ]
    if pd.notna(row.get("ai_reason")):
        children.append(section("AI Summary", {
            "Summary": row.get("ai_reason", "â€”"),
        }))

    def clean_score_text(value):
        return f"{float(value):.1f}/10" if pd.notna(value) else "—"

    children = [
        children[0],
        section("Vulnerability (Tenable.io)", {
            "Model Risk Score": clean_score_text(row.get("tenable_risk_score")),
            "Model Priority": row.get("tenable_priority_level", "—"),
            "Model Remediation": row.get("tenable_remediation", "—"),
        }),
        section("Threat (Microsoft Defender)", {
            "Model Risk Score": clean_score_text(row.get("defender_risk_score")),
            "Model Priority": row.get("defender_priority_level", "—"),
            "Model Remediation": row.get("defender_remediation", "—"),
        }),
        section("Logs & Anomaly (Splunk)", {
            "Model Risk Score": clean_score_text(row.get("splunk_risk_score")),
            "Model Priority": row.get("splunk_priority_level", "—"),
            "Model Remediation": row.get("splunk_remediation", "—"),
        }),
        section("Patch Status (IBM BigFix)", {
            "Model Risk Score": clean_score_text(row.get("bigfix_risk_score")),
            "Model Priority": row.get("bigfix_priority_level", "—"),
            "Model Remediation": row.get("bigfix_remediation", "—"),
        }),
    ]
    if pd.notna(row.get("ai_reason")):
        children.append(section("AI Summary", {
            "Summary": row.get("ai_reason", "—"),
        }))

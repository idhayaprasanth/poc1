...
def _source_available(value) -> bool:
...
def build_asset_table(table_id: str, df: pd.DataFrame) -> dash_table.DataTable:
    ...
    """
    cols_display = [
        "asset_id", "asset_name", "risk_score", "risk_level", "tenable_available", "splunk_available", "remediation", "issue_status",
    ]
    ...
    col_names = {
        ...
        "risk_level": "Level",
        "tenable_available": "Tenable",
        "splunk_available": "Splunk",
    ...
    if "tenable_available" not in df.columns:
        df["tenable_available"] = df.get("tenable_raw", pd.Series([pd.NA] * len(df), index=df.index)).map(_source_available)
    if "splunk_available" not in df.columns:
        df["splunk_available"] = df.get("splunk_raw", pd.Series([pd.NA] * len(df), index=df.index)).map(_source_available)

    df["tenable_available"] = df["tenable_available"].map(lambda value: "Available" if bool(value) else "Not Available")
    df["splunk_available"] = df["splunk_available"].map(lambda value: "Available" if bool(value) else "Not Available")
    ...
    return dash_table.DataTable(
        ...
        style_data_conditional=[
            ...
            {"if": {"filter_query": '{tenable_available} = "Available"', "column_id": "tenable_available"}, "color": COLORS["low"], "fontWeight": "700"},
            {"if": {"filter_query": '{splunk_available} = "Available"', "column_id": "splunk_available"}, "color": COLORS["low"], "fontWeight": "700"},
            {"if": {"filter_query": '{tenable_available} = "Not Available"', "column_id": "tenable_available"}, "color": COLORS["medium"], "fontWeight": "700"},
            {"if": {"filter_query": '{splunk_available} = "Not Available"', "column_id": "splunk_available"}, "color": COLORS["medium"], "fontWeight": "700"},

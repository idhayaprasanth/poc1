...
class DGXSparkServerClient:
    def __init__(self):
        ...
        self.aws_region = (
            str(os.getenv("AWS_REGION", "us-gov-west-1")).strip()
            or "us-gov-west-1"
        )
        self.endpoint_label = (
    ...
    def _compact_prompt_value(value, *, max_depth: int = 3, max_items: int = 6, max_chars: int = 300):
        ...
        if isinstance(value, dict):
            ...
            for index, (key, item) in enumerate(value.items()):
                if index >= max_items:
                    ...
                    break
                compacted[key] = DGXSparkServerClient._compact_prompt_value(
                    item,
        ...
        if isinstance(value, list):
            ...
            for index, item in enumerate(value):
                if index >= max_items:
                    compacted_list.append("__truncated__")
                    break
                compacted_list.append(
                    DGXSparkServerClient._compact_prompt_value(
                        item,
                        max_depth=max_depth - 1,
                        max_items=max_items,
                        max_chars=max_chars,
    ...
    def _resolve_max_new_tokens(self, *, inputs_text: str) -> int:
        hard_limit = 4096
        safety_margin = 128
        estimated_input_tokens = self._estimate_token_count(inputs_text)
        available_tokens = hard_limit - estimated_input_tokens - safety_margin
    ...
    def generate_asset_analysis(self, *, asset_record: dict) -> dict:
        ...
        if not self.enabled():
            raise DGXSparkServerInvocationError(
                "DGX Spark Server endpoint is not configured. Set DGX_SPARK_SERVER_ENDPOINT_URL or DGX_SPARK_SERVER_ENDPOINT_NAME."
            )
    ...
    def generate_security_answer(
        ...
        *,
        ...
        if not self.enabled():
            return "DGX Spark Server endpoint is not configured. Set DGX_SPARK_SERVER_ENDPOINT_URL or DGX_SPARK_SERVER_ENDPOINT_NAME."
